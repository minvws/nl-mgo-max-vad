...
.venv/touchfile: ## Includes workaround for https://github.com/xmlsec/python-xmlsec/issues/320
	...
	. .venv/bin/activate && ${env} pip install --no-binary lxml lxml==4.9.3 --force-reinstall
	. .venv/bin/activate && ${env} pip install --no-binary xmlsec xmlsec==1.3.14 --no-deps --force-reinstall
