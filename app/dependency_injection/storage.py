...
class Storage(containers.DeclarativeContainer):
    ...
    auth_session_cache = providers.Singleton(
        AuthSessionCache,
        cache=cache,
        sym_encryption_service=encryption_services.user_authentication_encryption_service,
        cache_ttl_seconds=config.auth_session.cache_ttl_seconds.as_int(),
    )
