...
class TestUserinfoProvider:
    def test_exchange_bsn(self, mocker: MockerFixture, faker: Faker) -> None:
        ...
        auth_session_id: str = faker.uuid4(cast_to=str)
        ...
        userinfo_dto = userinfo_provider.exchange_bsn(
            bsn=bsn, auth_session_id=auth_session_id
        )
...
class TestVadUserinfoService:
    def test_request_userinfo_for_saml_artifact(
        ...
        mock_userinfo_dto.model_dump_json.return_value = '{"foo":"bar"}'

        mocker.patch("app.userinfo.services.uuid.uuid4", return_value=auth_session_id)

        userinfo = vad_userinfo_service.request_userinfo_for_saml_artifact(
        ...
        assert (
            Userinfo(
                ...
                content_type=VadUserinfoService.CONTENT_TYPE,
            )
            == userinfo
        )
        ...
        mock_userinfo_provider.exchange_bsn.assert_called_once_with(
            bsn, str(auth_session_id)
        )
        mock_userinfo_dto.model_dump_json.assert_called_once()
    ...
    def test_request_userinfo_for_session_returns_userinfo(
        ...
        mock_userinfo_provider.exchange_session.return_value = UserInfoDTO(
            rid=rid, person=person, sub=auth_session_id
        )

        auth_session = AuthSession(auth_session_id)
        userinfo = vad_userinfo_service.request_userinfo_for_session(auth_session)
        ...
        assert VadUserinfoService.CONTENT_TYPE == userinfo.content_type
        assert auth_session_id == userinfo.auth_session_id
