...
@pytest.mark.parametrize("with_auth_session", [False, True])
def test_present_login_options_or_authorize(with_auth_session: bool, faker: Faker):
    auth_session = None
    if with_auth_session:
        auth_session = AuthSession(uuid4())

    mock_rate_limiter = MagicMock()
    mock_authentication_handler_factory = MagicMock()
    mock_pyop_provider = MagicMock()
    mock_userinfo_service = MagicMock()
    mock_authentication_cache = MagicMock()
    mock_response_factory = MagicMock()
    oidc_provider = create_oidc_provider(
        rate_limiter=mock_rate_limiter,
        authentication_handler_factory=mock_authentication_handler_factory,
        pyop_provider=mock_pyop_provider,
        userinfo_service=mock_userinfo_service,
        authentication_cache=mock_authentication_cache,
    ...
    authorize_request = MagicMock()
    authorize_request.client_id = "client_id"
    authorize_request.redirect_uri = faker.uri()
    authorize_request.response_type = faker.word()
    authorize_request.nonce = faker.uuid4()
    authorize_request.scope = faker.word()
    authorize_request.state = faker.sha256()
    authorize_request.code_challenge = faker.sha256()
    authorize_request.code_challenge_method = faker.word()
    pyop_authorization_request = MagicMock()
    pyop_authorization_response = MagicMock()
    pyop_authorization_response["code"] = faker.uuid4()
    userinfo = faker.pystr()
    client = {"name": "name"}
    ...
    with patch.object(
        ...
        mock_pyop_provider.parse_authentication_request.return_value = (
            pyop_authorization_request
        )

        if with_auth_session:
            mock_pyop_provider.authorize.return_value = pyop_authorization_response
            mock_userinfo_service.request_userinfo_for_session.return_value = userinfo

        oidc_provider.present_login_options_or_authorize(
            request=request,
            authorize_request=authorize_request,
            auth_session=auth_session,
        )

        # present_login_options_or_authorize()
        ...
        # _authorize()
        mock_rate_limiter.validate_outage.assert_called()
        mock_pyop_provider.parse_authentication_request.assert_called()
        mock_rate_limiter.ip_limit_test.assert_called()

        expected_call_count = 0 if with_auth_session else 1
        assert create_authentication_meta.call_count == expected_call_count
        assert (
            mock_authentication_handler_factory.create.call_count == expected_call_count
        )
        assert (
            mock_authentication_cache.create_randstate.call_count == expected_call_count
        )
        assert (
            mock_authentication_cache.cache_authentication_request_state.call_count
            == expected_call_count
        )

        if with_auth_session:
            mock_pyop_provider.authorize.assert_called_with(
                pyop_authorization_request, "_"
            )
            mock_userinfo_service.request_userinfo_for_session.assert_called_with(
                auth_session
            )
            assert (
                mock_authentication_cache.cache_acs_context.call_args[0][0]
                == pyop_authorization_response["code"]
            )
            acs_context = mock_authentication_cache.cache_acs_context.call_args[0][1]
            assert isinstance(acs_context, AcsContext)
            assert acs_context.client_id == "client_id"
            assert acs_context.authentication_method == "a"
            assert acs_context.userinfo == userinfo
            pyop_authorization_response.request.assert_called_with(
                pyop_authorization_request["redirect_uri"], False
            )
            mock_response_factory.create_redirect_response.assert_called()
        else:
            mock_pyop_provider.authorize.assert_not_called()
            mock_userinfo_service.request_userinfo_for_session.assert_not_called()
            mock_authentication_cache.cache_acs_context.assert_not_called()
            mock_response_factory.create_redirect_response.assert_not_called()
...
def test_authorize_without_client():
    ...
    with pytest.raises(ServerErrorException):
        oidc_provider._authorize(
            request=request,
            authorize_request=authorize_request,
            login_method=LoginMethod(name="login_option", type=LoginMethodType.OIDC),
            auth_session=None,
        )
